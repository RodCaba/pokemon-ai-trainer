...
//
// Output: data/reg-m-a/db.sqlite (overwritten via tmp + rename for atomicity).
 ...
 *
 * The build is deterministic: same inputs → byte-identical SQLite. Atomic write
 * via `${dbPath}.tmp` then rename. `PRAGMA journal_mode=DELETE; VACUUM;` at the
 * end ensures stable file bytes.
 *
...
export async function buildRegMA(
  ...
  // 2. Insert items (sorted by id for determinism).
  ...
  // 4. Insert moves (sorted). Accuracy from @pkmn/dex (Champions doesn't expose it).
  ...
  db.$client.transaction(() => {
    for (const m of sortedMoves) {
      ...
      // Champions calc data omits `category` for non-damaging moves (it doesn't
      // need it). Default to "Status" when missing AND basePower is 0/missing.
  ...
  // 5. Insert species + their related rows. Sort by id for determinism.
  ...
  db.$client.transaction(() => {
    for (const sp of sortedSpecies) {
      ...
      // Ability slots: prefer @pkmn/dex (full list); fall back to Champions calc-default.
  ...
  // 5b. Ingest curated sample sets from the committed SETDEX_CHAMPIONS snapshot.
  // The snapshot is fetched/refreshed via `pnpm data:refresh:reg-m-a`. If the
  // file is missing, we log a warning and skip — the build still completes (the
  // coverage test asserts sample_sets count, so missing data surfaces in tests).
  ...
  // 6. Finalize for determinism.
  db.$client.exec("PRAGMA journal_mode=DELETE; VACUUM;");
  db.$client.close();

  // Atomic rename.
  if (existsSync(dbPath)) unlinkSync(dbPath);
  // Use Node fs/promises rename for cross-fs safety. better-sqlite3 has no
  // direct rename; we use the lower-level fs API.
