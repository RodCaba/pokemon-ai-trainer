...
describe("knowledge repo (VGC-T38–VGC-T49)", () => {
  ...
  it("VGC-T41. throws KnowledgeStorageError on dimension mismatch", () => {
    ...
    try {
      ...
      // 512-dim vector instead of 1024 — defensive guard.
      const wrong = new Float32Array(512);
      let thrown: unknown;
      try {
        knowledge.upsertArticleChunks(db, {
          article_slug: "speed-control",
  ...
  it("VGC-T46. search returns top-k by cosine on seeded vectors (deterministic fixture)", () => {
    const db = open(":memory:");
    try {
